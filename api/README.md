...
MAMP defaults are configured in `config.php`: MySQL host `127.0.0.1`, port `8889`, database `angel`, user `root`, and password `root`. Set `ELYS_DB_HOST`, `ELYS_DB_PORT`, `ELYS_DB_NAME`, `ELYS_DB_USER`, `ELYS_DB_PASS`, and `ELYS_ADMIN_TOKEN` in the server environment for production. The required tables are created automatically on the first API request.
